...
# version 062
#         prise en compte des nouvelles api
...
def get_challenge(freebox_app_id):
    api_url = '%s/v8/login/authorize/%s' % (ENDPOINT, freebox_app_id)
...
def open_session(password, freebox_app_id):
    api_url = '%s/v8/login/session/' % ENDPOINT
...
def close_session(headers):
    api_url = '%s/v8/login/logout/' % ENDPOINT
...
def get_internal_disk_stats(headers):
    api_url = '%s/v8/storage/disk/' % ENDPOINT
...
def get_connection_stats(headers):
    api_url = '%s/v8/connection/' % ENDPOINT
...
def get_ftth_status(headers):
    api_url = '%s/v8/connection/ftth/' % ENDPOINT
...
def get_xdsl_status(headers):
    api_url = '%s/v8/connection/xdsl/' % ENDPOINT
...
def get_lteconfig_status(headers):
    api_url = '%s/v8/connection/lte/config' % ENDPOINT
...
def get_switch_status(headers):
    api_url = '%s/v8/switch/status/' % ENDPOINT
...
# -P => update pour avec POP
    api_url = '%s/v8/switch/port/%s/stats/' % (ENDPOINT, port)
...
def get_lan_config(headers):
    api_url = '%s/v8/lan/config/' % ENDPOINT
    r = requests.get(api_url, headers=headers)
...
def get_wifi_stats(headers, num):
    api_url = '%s/v8/wifi/ap/%s/stations' % (ENDPOINT, num)
...
def get_lan_interfaces(headers):
    api_url = '%s/v8/lan/browser/interfaces/' % ENDPOINT
    r = requests.get(api_url, headers=headers)
...
def get_interfaces_hosts(headers, interf):
    api_url = '%s/v8/lan/browser/%s/' % (ENDPOINT, interf)
...
def get_static_dhcp(headers):
    api_url = '%s/v8/dhcp/static_lease/' % ENDPOINT
    r = requests.get(api_url, headers=headers)
...
def get_dynamic_dhcp(headers):
    api_url = '%s/v8/dhcp/dynamic_lease/' % ENDPOINT
    r = requests.get(api_url, headers=headers)
...
if __name__ == '__main__':
    ...
    ENDPOINT="http://"+args.Endpoint+"/api/"
    auth = get_auth()
